import { CheckCircle, RadioButtonUnchecked } from "@mui/icons-material";
import {
  Radio,
  FormControl,
  FormControlLabel,
  FormLabel,
  RadioGroup,
} from "@mui/material";
import React, { useState } from "react";
import { useNavigate } from "react-router-dom";
import Accordion from "@mui/material/Accordion";
import AccordionDetails from "@mui/material/AccordionDetails";
import AccordionSummary from "@mui/material/AccordionSummary";
import Typography from "@mui/material/Typography";
import ExpandMoreIcon from "@mui/icons-material/ExpandMore";
import { getText2 } from "../locale";

const Prarobotka2 = () => {
  const nav = useNavigate();
  const [expanded, setExpanded] = useState(false);

  const handleChange = (panel) => (event, isExpanded) => {
    setExpanded(isExpanded ? panel : false);
  };
  return (
    <div className="Pro1">
      <div className="pro_box">
        <div className="seriy f600 s36 ta">
          Assalomu aleykum bizning aziz ishtirokchimiz.
        </div>
        <div className="color_main f600 ta s36 mt-4">
          BahtWoman Baxt maktabining o'quv platformasiga <br /> Xush Kelibsiz.
        </div>
        <div className="f600 mt-5 s24 ta">
          Hozirda biz sizni otangiz bilan bo'lgan munosabatlaringiz sizning
          hayotingizga qanday ta'sir o'tkazganini kuchli tahlil va prorabotkalar
          asosida ishlab chiqyapmiz.
        </div>
        <div className="f600 seriy s36 mt-5 ta">
          Demak, savolni diqqat bilan o'qiymiz.
        </div>
        <div className="f500 ta seriy s28 mt-3">
          Siz 18 yoshingizgacha otasiz o'sgan qiz bo'lganmisiz: masalan,
          ota-onangiz 18 yoshingizgacha ajrashib ketishgan bo'lishsa, yoki
          dadangiz vafot etgan bo'lsa, yoki siz boshqa sabablarga ko'ra
          dadangizni mutlaqo ko'rmasdan katta bo'lgan qiz bo'lganmisiz?
        </div>
        <div className="list_item">
          <img src="/icon_cor.png" alt="" />
          <div className="f500 seriy s24">
            Hozir siz quyida keltirilgan 5ta muhim jihat to'g'risida o'ylab
            ko'rasiz, qaysi biri sizga tegishliligini o'zingiz uchun
            bloknotingizda belgilaysiz.
          </div>
        </div>
        <div className="list_item">
          <img src="/icon_cor.png" alt="" />
          <div className="f500 seriy s24">
            Shundan so'ng har bir punkt bo'yicha (ichingiz bo'shagunga qadar)
            yodingizga tushgan barcha qalb kechinmalaringiz yuzasidan kichik
            yoki katta hikoya yozib chiqishingiz kerak bo'ladi.
          </div>
        </div>
        <div className="list_item">
          <img src="/icon_cor.png" alt="" />
          <div className="f500 seriy s24">
            Undan keyin esa aynan sizga tegishli gapni prorabotkalarni ichidan
            topib, yozib chiqasiz.
          </div>
        </div>
        <div className="f600 seriy s36 mt-5 ta">Demak o'qishni boshlaymiz.</div>
        <div className="s24 mt-4 f500">
          <span className="kok s28 pl">Birinchisi</span> – Agar siz otasiz
          o'sgan bo'lsangiz, katta ehtimol bilan, siz o'zingizni boshqa
          qizlardan ko'ra baxtsizroq his qilagansiz, to'g'rimi?!{" "}
        </div>
        <div className="s24 mt-4 f500">
          <span className="kok s28 pl">Chunki</span> sizni hayotingizdagi ilk
          erkak, ya'ni otangiz sizni tashlab ketgan. Bu tuyg'uni siz dadangiz
          ketgan ilk kunlaridan his qilishni boshlaganingiz ajab emas.
          to'g'rimi?!{" "}
        </div>
        <div className="s24 mt-4 f500">
          <span className="seriy s28 pl">Bu faktor esa</span> siz hayotingizni
          qolgan yillari davomida – to hozirgi kungacha “yo'qotish” qo'rquvi
          bilan kurashib kelganingizni va boshqa ayol-qizlar sizdan baxtliroq
          ekanini o'ylab yashashingizni anglatadi, to'g'rimi?!
        </div>
        <div className="s24 mt-4 f500">
          <span className="color_main s28 pl">Nima deb o'ylaysiz,</span> hozirda
          siz erkaklardan / yigitlardan / va hatto o'g'illaringizdan ular o'zini
          sizga to'liq baxshida etishini va bor vaqtini / yoki mehr-muhabbatini
          faqat va faqat sizga bag'ishlashini talab qilasizmi / yoki oldin talab
          qilganmisiz / yoki juda bo'lmaganda shu haqida orzu qilasizmi?
        </div>
        <div className="s24 mt-4 f500">
          <span className="seriy s28 pl">
            Psixologiyada – bu havotirli-yopishqoq shaxs turi deyiladi.
          </span>{" "}
          Ushbu turdagi odamlar ko'pincha o'ziga yaqin bo'lgan erkak sherigi
          bilan doimiy ravishda yaqin hissiy yaqinlik va bir tanu – bir jon
          bo'lib birlashish istagi zarurligini his qiladilar. Ular sevgan
          insonlaridan ajrab qolish borasida shunday katta shubha va qo'rquvlar
          bilan yashaydilar-ki, shuning uchun ham o'zlariga hissiy yaqin bo'lgan
          sheriklarining barcha xati-harakatlari va hatto his-tuyg'ularini
          o'zlarining qo'rquvlari hamda mafaatlaridan kelib chiqqan holda kuchli
          nazorat qilishga urinadilar.
        </div>
        <div className="s24 mt-4 f500">
          <span className="fil s28 pl">Biroq bitta narsani bilib oling</span> –
          agar siz shunday turdagi qiz yoki ayol bo'lsangiz, demak aynan shu
          sababli ham siz ishlashga toqati yo'q va ko'plab vaqtini siz bilan
          o'tkazishni istaydigan (va ko'p hollarda ruhiyati nosog'lom) erni yoki
          yigitni hayotingizga jalb qilgan bo'lishingiz ham mumkin / yoki
          shunday o'g'ilni voyaga yetkazgan bo'lishingiz mumkin.
        </div>
        <div className="list_item pl50 s24 mt-4 f500">
          <img src="/img/1.png" alt="" />
          <div>
            Va tabiiy holda siz hozir ham boshqa ayollar bilan o'zingizni
            solishtirib, ular sizdan baxtliroq ekanini o'ylab, siqilib
            yashayotganingiz ehtimoldan holi emas.
          </div>
        </div>
        <div className="list_item pl50 s24 mt-4 f500">
          <img src="/img/1.png" alt="" />
          <div>
            Aslida esa sizning Ong ostingiz va bolalikda jarohatlangan
            ruhiyatingiz ushbu hayotiy stsenariyni{" "}
            <span className="">
              “Nima bo'lsa ham, yomon bo'lsa ham, eng asosiysi – yaxshi ko'rgan
              erkagim yonimda bo'lsa bo'ldi”
            </span>{" "}
            degan shior ostida o'ylab topgan, xolos.
          </div>
        </div>
        <div className="s24 mt-4 f500">
          <span className="seriy s28 pl">Ikkinchisi</span> – Siz romantikani,
          erkaklar sizga bo'ysunishini va sizga ixlosmand bo'lishini naqadar
          istaysiz va bularni olmasangiz, qanchalik siqilib, azob chekasiz?
        </div>
        <div className="list_item pl50 s24 mt-4 f500">
          <img src="/img/3.png" alt="" />
          <div>
            Agar javobingizda
            <span className="seriy">
              “Ha, men xuddi shundayman va agarda ulardan kutgan mehrimni
              ololmasam, qattiq siqilib, depressiyaga tushib qolaman”
            </span>{" "}
            deb javob bergan bo'lsangiz, demak siz Ong osti darajasida bolalikda
            otangizdan to'liq ololmagan hissiyotlaringizni turmush o'rtog'ingiz
            / yoki yigitingizdan olishga qattiq urinyaptganingizni anglatadi.
          </div>
        </div>
        <div className="list_item pl50 s24 mt-4 f500">
          Shunday bo'lsa-da, siz uning o'zini ayblash uchun sabab topasiz.
          Negaki Ong osti darajasida siz hali ham otangizni u sizning
          hayotingizdan chiqib ketganligi bois sizga maksimal darajada mehr bera
          olmagani uchun ayblamoqchi bo'lasiz va bu ayblovlarni hayotingizdagi
          boshqa erkak personajga ko'chirib o'tyaptgan bo'lasiz.
        </div>
        <div className="list_item pl50 s24 mt-4 f500">
          <img src="/img/1.png" alt="" />
          <div>
            <span className="seriy">Bu degani, siz</span> yaxshi ko'rgan
            erkagingizga aybdorlik tuyg'usini singdirish orqali / uni kuchli
            vijdon azobida qoldirish orqali undan mehr talab qilmoqchi bo'lasiz.
            Chunki shunday qilsam, u menga ko'proq mehr beradi va tashlab keta
            olmaydi, deb o'ylaysiz.
          </div>
        </div>
        <div className="s24 mt-4 f500">
          <span className="fil s28 pl">Uchinchisi</span> – Siz turmush
          o'rtog'ingiz, yigitingiz yoki sizga yaqin bo'lgan erkakni yo'qotishdan
          tez vahimaga tushasiz, to'g'rimi?! Chunki “Yaxshi ko'rgan insoningizni
          yo'qotish” modeli sizning Ong ostingizda majburiy darstur sifatida
          turibdi-ku.
        </div>
        <div className="s24 mt-4 f500">
          <span className="seriy s28 pl">Endi o'ylab ko'ring</span> - Siz
          yo'qotish hisidan qattiq qo'rqishingiz oqibatida sizning hayotingizda
          er (yoki shunchaki erkak) bo'lishi uchungina siz:
        </div>
        <div className="list_item pl50 s24 mt-4 f500">
          <img src="/icon_cor.png" alt="" />
          <div>haqorat va so'kishlarga,</div>
        </div>
        <div className="list_item pl50 s24 mt-4 f500">
          <img src="/icon_cor.png" alt="" />
          <div>jismoniy zo'ravonlikka,</div>
        </div>
        <div className="list_item pl50 s24 mt-4 f500">
          <img src="/icon_cor.png" alt="" />
          <div>uzoq muddatli ayriliqqa,</div>
        </div>
        <div className="list_item pl50 s24 mt-4 f500">
          <img src="/icon_cor.png" alt="" />
          <div>pulsizlik va sovg'a olmaslikka,</div>
        </div>
        <div className="list_item pl50 s24 mt-4 f500">
          <img src="/icon_cor.png" alt="" />
          <div>yoki kuchli rashk va xiyonatga,</div>
        </div>
        <div className="s24 mt-4 f500">
          hamda hayotning boshqa shunga o'xshash ko'plab adolatsizliklariga
          chidaganmisiz?
        </div>
        <div className="f600 seriy s36 mt-5 ta">
          Kelajakda ham chidashga rozimisiz?
        </div>
        <div className="s24 mt-4 f500">
          <span className="fil s28 pl">Gap shundaki</span> – sizning miyangiz
          shunday hayotiy pozitsiyani himoya mexanizmi tariqasida ishlab chiqqan
          bo'lishi mumkin. Ya'ni u – Men sevikli erkagimni yo'qotishni yana bir
          bor his qilmaslik uchun barcha qiyinchiliklarga chidashga, qurbon
          rolida hayot kechirishga ham roziman – deydi.
        </div>
        <div className="list_item pl50 s24 mt-4 f500">
          <img src="/img/3.png" alt="" />
          <div>
            Va, tabiiy ravishda, siz oilangizni saqlab qolish uchun, aniqrog'i
            soxta mustahkam nikohni ushlab qolish maqsadida o'zingizni hammaga
            baxtli qilib ko'rsatishga, aslida esa baxtsizlik gulhanida
            kuyib-pishgan holda butun umr kurashishga rozi bo'lasiz.
          </div>
        </div>
        <div className="s24 mt-4 f500">
          <span className="toq_fil s28 pl">To'rtinchisi</span> – Hayotingiz
          davomida yigitlar / erkaklar / va hatto o'g'illaringiz sizni tashlab
          ketganlarmi / yoki hayotingizdan chiqib ketishganmi?
        </div>
        <div className="list_item pl50 s24 mt-4 f500">
          <img src="/img/3.png" alt="" />
          <div>
            Masalan, eringiz / yoki o'g'lingiz chet davlatga yoki boshqa
            shaharga ishlashga / yoki o'qishga ketib (va hattoki o'sha yerda
            qolib ketgan bo'lishi mumkin);
          </div>
        </div>
        <div className="list_item pl50 s24 mt-4 f500">
          <img src="/img/3.png" alt="" />
          <div>Yoki u qamalib ketgan bo'lishi mumkin;</div>
        </div>
        <div className="list_item pl50 s24 mt-4 f500">
          <img src="/img/3.png" alt="" />
          <div>Yoki bedarak yo'qolib qetgan bo'lishi mumkin;</div>
        </div>
        <div className="list_item pl50 s24 mt-4 f500">
          <img src="/img/3.png" alt="" />
          <div>Yoki olamdan o'tib qolgan bo'lishi mumkin;</div>
        </div>
        <div className="list_item pl50 s24 mt-4 f500">
          <img src="/img/3.png" alt="" />
          <div>Yoki shunchaki siz bilan ajrashib ketgan bo'lishi mumkin.</div>
        </div>
        <div className="s24 mt-4 f500">
          Chunki sizning bolalagingizdagi nozik ruhiyatingiz
          <span className="toq_fil s28 pl">
            “Yaxshi ko'rgan eng yaqin va eng birinchi erkagim hayotimdan yo'q
            bo'lib chiqib ketdi, demak qolganlari ham chiqib ketadi”
          </span>{" "}
          degan dasturni bilvosita Ong ostingizga muxrlab qoldirgan.
        </div>
        <div className="list_item pl50 s24 mt-4 f500">
          <img src="/img/3.png" alt="" />
          <div>
            Bu Ong osti dasturi ishlayaptganini qanday bilsangiz bo'ladi? Bu
            uchun quyidagi kalit haqida o'ylab ko'ring –{" "}
          </div>
        </div>
        <div className="list_item s24 mt-4 f500">
          <img src="/img/2.png" alt="" />
          <div>
            Mana shu erkaklar hayotingizdan chiqib ketishgan holatlarda siz
            bolalikda dadangizni yo'qotgan paytingizdagi aynan o'sha o'xshash
            his-tug'ular va kechinmalarni boshdan kechirganmisiz?
            <div className="mt-2 toq_fil s28 pl">
              Solishtirib ko'ring-chi!
            </div>{" "}
          </div>
        </div>
        <div className="list_item pl50 s24 mt-4 f500">
          <img src="/img/3.png" alt="" />
          <div>
            Ya'ni biz hozir miyadagi ongsiz relslarning ishi haqida siz bilan
            gaplashyapmiz – u yoki bu travmatik his-tuyg'uning qayta-qayta faqat
            boshqa personajlar orqali qaytarilishi haqida.
          </div>
        </div>
        <div className="s24 mt-4 f500">
          <span className="color_main s28 pl">Va beshinchisi</span> – Siz xuddi
          o'zingizga o'xshagan otasidan yoki onasidan ayrilib qolgan, ya'ni
          yarim-etim yigit / yoki erkakni uchratib, u bilan turmush qurganmisiz
          / yoki turmush qurish istagida yashamayapsizmi?
        </div>
        <div className="f600 color_main s36 mt-5 ta">
          Demak, savolni diqqat bilan o'qiymiz.
        </div>
        <div className="list_item  s24 mt-4 f500">
          <img src="/img/3.png" alt="" />
          <div>
            Otasini yo'qotgan qizlar ko'pincha onasini yo'qotgan yigit yoki
            erkaklar bilan aloqaga kirishadi.{" "}
          </div>
        </div>{" "}
        <div className="list_item  s24 mt-4 f500">
          <img src="/img/3.png" alt="" />
          <div>
            Onasini yo'qotgan qizlar esa ko'pincha otasini yo'qotgan yigit va
            erkaklarni hayotlariga jalb qilishadi.
          </div>
        </div>
        <div className="list_item pl s24 mt-4 f500">
          <img src="img/2.png" alt="" />
          <div>
            <span className="color_main">
              Va bu yerda eng achinarlisi shundaki
            </span>
            sizning erkagingiz ham sizni yo'qotishdan qo'rqib (negaki bolaligida
            u ham yaqin odamini yo'qotgan va hayotida bu narsa boshqa
            qaytarilmasligi uchun u o'zini nazorat qila olmaydigan axmoqlik
            darajasigacha borishini ehtimoli katta), sizni nosog'lom darajada
            rashk qilib, asabingizni ko'p yeb tamom qilmaganmi / yoki
            qilmayaptimi?
          </div>
        </div>
        <div className="f600 seriy s36 mt-5 ta">
          Demak, bizning variant javoblarimiz tugadi va biz siz bilan
          prorabotkalarimizni yozishga o'tamiz.
        </div>
        <div
          onClick={() => nav("/what-pro")}
          className="prorabotka_what tort f600 color_main s36 mt-5 ta"
        >
          PRORABOTKA – BU NIMA?
        </div>
        <div className="tort mt-5">
          <div className=" f600 kok s36 mt-5 ta">Prorabotka:</div>

          <div className=" f600 kok s24 mt-5 ta">
            Demak, hozirda bloknotingizni yangi varog'ini ochib, quyidagilarni
            ichidan FAQAT o'zingizga tegishli prorabotkani (ya'ni anglash,
            tushunish va kechirimlarni) topib, yozib chiqasiz.
          </div>
          <div className=" f600 color_main s24 mt-5 ta">
            Keyin esa pozitiv prorabotkalarga o'tamiz.
          </div>
          <div className="list_item  s24 mt-4 f500">
            <img src="/icon_cor.png" alt="" />
            <div>Salom, dada.</div>
          </div>
          <div className="list_item  s24 mt-4 f500">
            <img src="/icon_cor.png" alt="" />
            <div>
              Bolalik / o'smirlik chog'imda, ya'ni mening Ong ostim aynan
              shakllanyaptgan vaqtda men sizsiz o'sganim chog'larimni eslar
              ekanman, o'sha paytlarda men o'zimni boshqa qizlardan ko'ra
              baxtsizroq his qilar edim.
            </div>
          </div>
          <div className="list_item  s24 mt-4 f500 ms-5">
            <img src="/img/1.png" alt="" />
            <div>
              Chunki dugonalarimning dadasi bo'lar edi, meni esa hayotimdagi ilk
              erkak, ya'ni siz meni hayotimda yo'q edingiz. Mening otam yo'q –
              degan bu alamli tuyg'uni men siz ketgan ilk kunlaringizdan his
              qilishni boshlaganman va u tuyg'u ortsa ortgan-ki, lekin
              kamaymagan.
            </div>
          </div>
          <div className="list_item  s24 mt-4 f500">
            <img src="/icon_cor.png" alt="" />
            <div>
              Bu faktor shunga olib keldiki, men hayotimning qolgan yillari
              davomida – to hozirgi kungacha “yo'qotish” qo'rquvi bilan kurashib
              kelaman va hali ham boshqa ayol-qizlar bilan o'zimni solishtiri,
              ular mendan sizdan baxtliroq ekanini o'ylab siqilaman!
            </div>
          </div>
          <div className="list_item  s24 mt-4 f500 pl">
            <img src="/img/2.png" alt="" />
            <div>
              <span className="seriy f600">Lekin eng asosiysi </span> – hozirda
              men erkaklardan / yigitlardan / va hatto o'g'illarimdan ular
              o'zini menga to'liq baxshida etishini va bor vaqtini / yoki
              mehr-muhabbatini faqat va faqat menga bag'ishlashini talab qilaman
              / yoki oldin talab qilganman va ko'p yuraklarini siqqanman / yoki
              juda bo'lmaganda shu haqida orzu qilib yashayman.
            </div>
          </div>
          <div className="list_item  s24 mt-4 f500 pl50 ms-5">
            <img src="/img/1.png" alt="" />
            <div>
              Ha, bugun men havotirli-yopishqoq shaxs turiga kiradigan qiz /
              yoki ayolga aylanib qolganimni aniqladim. Ha, men ko'pincha
              o'zimga yaqin bo'lgan erkak sherigim bilan doimiy ravishda yaqin
              hissiy yaqinlik va bir tanu – bir jon bo'lib birlashish istagi
              zarurligini his qilib yashashimni va bu ushbu tuyg'u / ehtiyoj
              nosog'lom ekanligini angladim.
            </div>
          </div>
          <div className="list_item  s24 mt-4 f500 pl50 ms-5">
            <img src="/img/1.png" alt="" />
            <div>
              Dada, men sizdan ajrab qolganim tufayli kelajakdagi hayotimda ham
              sevgan insonlarimdan ajrab qolish borasida katta shubha va
              qo'rquvlar bilan yashashimni va shuning uchun ham menga yaqin
              bo'lgan erkak personajlarning barcha xati-harakatlari va hatto
              his-tuyg'ularini o'zimning qo'qrquvlarim va mafaatlarimdan kelib
              chiqqan holda kuchli nazorat qilishga urinishimni bugun tushunib
              yetdim.
            </div>
          </div>
          <div className="list_item s24 mt-4 f500 ms-3 pl">
            <img src="img/2.png" alt="" />
            <div>
              <span className="color_main f600 s28 ">
                Biroq shu bilan birga men bugun{" "}
              </span>{" "}
              meni qiynayotgan savollardan biriga javob topdim – men o'zimga
              yaqin bo'lgan erkakni o'zimga qattiq bog'lab olishni istashim
              sababli ishlashga toqati yo'q va ko'plab vaqtini men bilan
              o'tkazishni istaydigan (va hatto ruhiyati nosog'lom) erni yoki
              yigitni hayotimga jalb qilgan ekanman. Yoki shunday o'g'ilni
              voyaga yetkazgan ekanman.
            </div>
          </div>
          <div className="list_item  s24 mt-4 f500 pl">
            <img src="/img/1.png" alt="" />
            <div>
              Va tabiiy holda men hozir ham boshqa ayollar bilan o'zimni
              solishtirib, ular mendan baxtliroq ekanini o'ylab, siqilib
              yashayotgan ekanman.
            </div>
          </div>
          <div className="list_item  s24 mt-4 f500 pl50">
            <img src="/img/3.png" alt="" />
            <div>
              Aslida esa mening Ong ostim va bolalikda jarohatlangan ruhiyatim
              ushbu hayotiy stsenariyni “Nima bo'lsa ham, yomon bo'lsa ham, eng
              asosiysi – yaxshi ko'rgan erkagim yonimda bo'lsa bo'ldi” degan
              shior ostida o'ylab topgan, xolos.
            </div>
          </div>
          <div className="list item s24 mt-4 f500 pl">
            <span className="color_main f600 s28 ">Bugun men nima uchun</span>{" "}
            romantikani, erkaklar menga bo'ysunishini va menga ixlosmand
            bo'lishini qattiq xohlashimni va bularni olmasam, siqilib azob
            chekishimni sabablarini tushunib yetyapman.
          </div>
          <div className="list_item  s24 mt-4 f500 pl">
            <img src="/img/1.png" alt="" />
            <div>
              Chunki men Ong osti darajasida bolalikda otamdan to'liq ololmagan
              hissiyotlarimni turmush o'rtog'im / yoki yigitimdan / yoki
              o'g'limdan / o'g'illarimdan olishga qattiq urinayotgan ekanman.
            </div>
          </div>
          <div className="list_item  s24 mt-4 f500 pl50 ms-5">
            <img src="/img/3.png" alt="" />
            <div>
              Shunday bo'lsa-da, men u(lar)ning o'zini ayblash uchun sabab
              topaman. Negaki Ong osti darajasida men hali ham otamni u mening
              hayotimdan chiqib ketganligi bois menga maksimal darajada mehr
              bera olmagani uchun ayblamoqchi bo'lar ekanman va bu ayblovlarni
              hayotimdagi boshqa erkak personajga ko'chirib o'tyaptganimni
              angladim.
            </div>
          </div>
          <div className="list_item  s24 mt-4 f500 pl">
            <img src="/img/1.png" alt="" />
            <div>
              Bu degani, men yaxshi ko'rgan erkagimga aybdorlik tuyg'usini
              singdirish orqali / uni kuchli vijdon azobida qoldirish orqali
              undan mehr talab qilmoqchi bo'lar ekanman. Chunki shunday qilsam,
              u menga ko'proq mehr beradi va tashlab keta olmaydi, deb o'ylasam
              kerak-da.
            </div>
          </div>
          <div className="list_item  s24 mt-4 f500 pl50 ms-5">
            <img src="/img/3.png" alt="" />
            <div>
              Axir, mening bolalikdagi ta'sirchan ruhiyatim va Ong ostim “Yaqin
              insonni yo'qotish” modelini ko'rgan-ku va bolalikda qolib ketgan
              qo'rquvlarim o'zining salbiy ishini qilyaptgan ekan.
            </div>
          </div>
          <div className="list_item s24 mt-4 f500 pl">
            <img src="/icon_cor.png" alt="" />
            <div>
              <span className="color_main f600 s28 ">Bugun men nima uchun</span>{" "}
              turmush o'rtog'imni, yigitimni yoki o'zimga yaqin bo'lgan erkakni
              yo'qotishdan vahimaga tushishim oqibatida hayotimda erkak bo'lishi
              uchungina haqorat, zo'ravonlik, qashshoqlik yoki uzoq muddatli
              ayriliq va hayotning boshqa adolatsizliklariga chidab
              kelayotganimning sabablarini tushunib yetdim.
            </div>
          </div>
          <div className="list_item  s24 mt-4 f500 pl">
            <img src="/img/1.png" alt="" />
            <div>
              Chunki Ong osti darajasida men otamni yo'qotganman va shu
              yo'qotish hayotimda boshqa qaytarilmasligi uchun men o'zimga
              baxtsiz hayotni ravo ko'rishga ham tayyor ekanman.
            </div>
          </div>
          <div className="list_item s24 mt-4 f500">
            <img src="/icon_cor.png" alt="" />
            <div>
              <span className="color_main f600 s28 ">
                Bugun men hayotimda nima uchun{" "}
              </span>
              xuddi o'zimga o'xshagan otasidan yoki onasidan ayrilib qolgan,
              ya'ni yarim-etim yigit / yoki erkakni uchratib, u bilan turmush
              qurganimni / yoki turmush qurish istagida yashamayaptganimni
              angladim.
            </div>
          </div>
          <div className="list_item  s24 mt-4 f500 pl50">
            <img src="/img/1.png" alt="" />
            <div>
              Chunki Ong osti darajasida insonlar o'zlariga o'xshagan
              sheriklarni jalb qilar ekanlar.
            </div>
          </div>
          <div className="list_item  s24 mt-4 f500 pl50">
            <img src="/img/1.png" alt="" />
            <div>
              Va mening yigitim yoki erim ham yaqin insonini yo'qotishdan
              qo'rqib (negaki bolaligida u ham kutmagan vaqtda otasini yoki
              onasini yo'qotgan va hayotida bu narsa boshqa qaytarilmasligi
              uchun u o'zini nazorat qila olmaydigan axmoqlik darajasigacha
              borib), meni nosog'lom darajada rashk qilib, asabimni ko'p yeb
              tamom qilganini asl tub sababini bugun tushunib yetdim.
            </div>
          </div>
          <div className="list_item s24 mt-4 f500">
            <img src="/icon_cor.png" alt="" />
            Bugun mening Ong ostim katta bo'lgandagi hayotimda sevilish modelini
            qaytara olmayaptgani uchun, dada, sizni, oyimni, o'zimni va
            Taqdirimni kechiraman.
          </div>
          <div className="list_item s24 mt-4 f500">
            <img src="/icon_cor.png" alt="" />
            Bugun mening Ong ostim katta bo'lgandagi hayotimda sevilish modelini
            qaytara olmayaptgani uchun, dada, sizni, oyimni, o'zimni va
            Taqdirimni kechiraman.
          </div>
          <div className="ta toq_fil f700 s30 mt-4">
            Endi esa pozitiv ustanovkalarni yozishga o'tamiz.
          </div>
          <div className="ta seriy f700 s28 mt-4">
            Yodingizda bo'lsa ushbu pozitiv ustanovkalarni siz 3 martadan 5
            marotabagacha yozishingiz kerak bo'ladi.
          </div>
          <div className="list_item s24 mt-4 f500">
            <img src="/icon_cor.png" alt="" />
            Bugundan boshlab, dada, mening sevgan erkakgim / yigitim va hatto
            o'g'illarim ham siz emas. Men siz bilan his qilgan yo'qotish
            tuyg'usini va qo'rquvini ular bilan bo'lgan munosabatlarga olib
            o'tishni boshqa istamayman.
          </div>
          <div className="list_item s24 mt-4 f500">
            <img src="/icon_cor.png" alt="" />
            Men boshqa sizning sevgi-muhabbatingizga ortiqcha muhtoj emasman.
          </div>
          <div className="list_item  s24 mt-4 f500 pl50 ms-5">
            <img src="/img/1.png" alt="" />
            <div>
              Chunki men ulg'aygan hamda sevgi-muhabbatni osonlik bilan
              hayotimga jalb qilayotgan shaxsga aylanyapman.
            </div>
          </div>
          <div className="list_item s24 mt-4 f500">
            <img src="/icon_cor.png" alt="" />
            Bugundan boshlab men yigitim / yoki turmush o'rtog'im / yoki sevgan
            insonim meni tashlab ketishidan qo'rqmayman ham, ortiq cho'chimayman
            ham.
          </div>
          <div className="list_item s24 mt-4 f500">
            <img src="/icon_cor.png" alt="" />
            Tashlab ketilish holati meni hayotimda boshqa qaytarilmaydi, chunki
            men bekamu-ko'st insonman va meni sevgan insonim men aslida mukammal
            qizligimni / yoki ayolligimni biladi va meni chin taqdirim shundoq
            ham men bilan qolishni istaydi.
          </div>
          <div className="list_item s24 mt-4 f500">
            <img src="/icon_cor.png" alt="" />
            Men o'zimga so'z beraman - Bundan buyon hayotim tamomila boshqacha
            bo'ladi.
          </div>
          <div className="list_item s24 mt-4 f500">
            <img src="/icon_cor.png" alt="" />
            Bugun Men o'tmishimni va meni tashlab ketgan barcha erkaklar/
            yigitlarni (shu yerda yigit / erkaklarning barchasini
            ism-familiyalarni yozib chiqing) kechirib, qo'yib yubordim va
            Ollohning yuksak energiyasidan yasalgan oltin qaychi bilan ular
            bilan bo'lgan barcha rishtalarni men hozirning o'zida va butun umrga
            kesib tashladim.
          </div>
          <div className="list_item  s24 mt-4 f500 pl50 ms-5">
            <img src="/img/1.png" alt="" />
            <div>
              Chunki ular mening Ong-osti dasturlarimni aks ettirgan personajlar
              bo'lgan ekan, xolos.
            </div>
          </div>
          <div className="list_item s24 mt-4 f500">
            <img src="/icon_cor.png" alt="" />
            Aynan hozir va bugun men fikrlarimning va idrokimning kuchi bilan
            hamda shu yozayotgan prorabotkalarim yordamida “Sevgan insonimni
            yo'qotish va tashlab ketilish” dasturini Ong ostimda butkul
            blokladim.
          </div>
          <div className="list_item s24 mt-4 f500">
            <img src="/icon_cor.png" alt="" />
            Men hozirda turmush o'rtog'im / yoki yigitim /yoki o'g'illarim bilan
            bog'liq bo'lgan barcha qo'rquvlarni qo'yib yuboryapman va ushbu
            qo'rquvlarimni butunlay bloklayapman.
          </div>

          <div className="ta s24 mt-4 f500">
            (<span className="f600 color_main">Eslatma: </span>
            Sevgan insoningiz bilan bog'liq qanday qo'rquvlaringiz bo'lsa,
            hammasini yozib chiqing va har bir yozgan qo'ruvingizni oxirida{" "}
            <span className=" color_main">
              “Qo'rquvim, men seni bloklayapman. Bugundan boshlab mening
              hayotimda miyamda aylanadigan ushbu negativ sahnalar hech-qachon
              kirib kelmaydi ham, qaytarilmaydi ham”
            </span>
            – deb yozing).
          </div>

          <div className="list_item s24 mt-4 f500">
            <img src="/icon_cor.png" alt="" />
            Bugundan boshlab men soxta baxtga to'la hayotdan voz kechaman va
            o'zimni yon-atrofdagi barcha insonlar bilan (tashlab ketilish
            qo'rquvidan qutilgan holda) samimiy tutishni boshlayapman.
          </div>
          <div className="list_item s24 mt-4 f500">
            <img src="/icon_cor.png" alt="" />
            Men erimni / yigitimni yoki o'g'illarimni (ular meni tashlab
            ketmasliklari uchun) ortiqcha oldimda ushlab o'tirish uchun
            turli-hil hayotiy voqealarni hamda omadsizliklarni jalb qilishni
            to'xtatdim.
          </div>
          <div className="list_item s24 mt-4 f500">
            <img src="/icon_cor.png" alt="" />
            Men ularni katta hayotga xursandchilik bilan qo'yib yuboryapman va
            ularni mustaqil, omadli, zo'r ishlarga joylashib ketgan va baxtli
            shaxslarga aylanyaptganlarini ko'rib, faqat xursand bo'lyapman.
          </div>

          <div className="list_item  s24 mt-4 f500 pl50 ms-5">
            <img src="/img/1.png" alt="" />
            <div>
              Chunki mening o'zimni hayotim ham turli-hil xursandchiliklarga
              to'lyapti va men yaqin erkaklarimni uyda yoki yonimda doimiy
              ravishda ushlab o'tirishim uchun menda xohish yoki ehtiyoj umuman
              qolmayapti.
            </div>
          </div>
          <div className="list_item  s24 mt-4 f500 pl50 ms-5">
            <img src="/img/1.png" alt="" />
            <div>
              Axir men o'zimga o'zim kerakli bo'lishni / o'zimni sevib, hurmat
              qilishni va mening hayotimda men o'zim borligimdan o'zimni baxtli
              his qilishni o'rganyapman.
            </div>
          </div>

          <div className="list_item s24 mt-4 f500">
            <img src="/icon_cor.png" alt="" />
            Bugundan boshlab meni sevgan yigitim / yoki erim meni ruhiyati
            nosog'lom insonlar kabi axmoqona rashk qilishni / yoki har bir
            ishimga aralashib, asabimni yeb tamom qilishni butkul to'xtatganini
            ko'ryapman.
          </div>
          <div className="list_item s24 mt-4 f500">
            <img src="/icon_cor.png" alt="" />
            Men uning ruhiyati tamomila sog'lomlashyaptganini guvohi bo'lyapman,
            hamda ushbu holat mening hayotimda boshqa qaytarilmasligi uchun
            Ollohga ming shukr deb aytyapman.
          </div>
          <div className="list_item s24 mt-4 f500">
            <img src="/icon_cor.png" alt="" />
            Hozir va shu yerda men hayotimning barcha jabhalariga Ollohning
            yuksak Muhabbat energiyasini jalb etyapman.
          </div>
          <div className="list_item s24 mt-4 f500">
            <img src="/icon_cor.png" alt="" />
            O'z idrokim va Yaratganning madadi bilan men misli magnit kabi
            hayotimga yaqinlarimning sadoqatini va haqiqiy sevgini jalb
            qilyapman.
          </div>
          <div className="list_item s24 mt-4 f500">
            <img src="/icon_cor.png" alt="" />
            Men bugun menga nisbatan sodiq / meni chin dildan sevadigan sog'lom
            ruhiyatli turmush o'rtog'im / yoki sevgan yigitim bor yangi olamimga
            qadam qo'yyapman va Ollohning o'zi men uchun chinakam yangi baxt
            yo'llarini ochyapti. Omin.
          </div>
        </div>
        <div className="umumiy">
          <div className=" seriy s28 mt-5 ta seriy">
            {getText2("pr1_341")} <br />
            <span className="f600">{getText2("pr1_342")} </span>
            <br />
            {getText2("pr1_343")} {getText2("pr1_344")}{" "}
          </div>
          <div className="tort seriy s30 ta mt-5 f600">
            {getText2("pr1_345")}
          </div>
          <Accordion
            className="mt-2"
            expanded={expanded === "panel1"}
            onChange={handleChange("panel1")}
          >
            <AccordionSummary
              expandIcon={<ExpandMoreIcon />}
              aria-controls="panel1bh-content"
              id="panel1bh-header"
            >
              <div className=" my_acc seriy s30  f600 ">
                {getText2("pr1_346")}
              </div>
            </AccordionSummary>
            <AccordionDetails>
              <Typography>
                <div className="toq_fil s24 mt-4 f500">
                  {getText2("pr1_351")}
                </div>
              </Typography>
            </AccordionDetails>
          </Accordion>
          <Accordion
            expanded={expanded === "panel2"}
            onChange={handleChange("panel2")}
            className="mt-2"
          >
            <AccordionSummary
              expandIcon={<ExpandMoreIcon />}
              aria-controls="panel1bh-content"
              id="panel1bh-header"
            >
              <div className="my_acc seriy s30  f600 ">
                {getText2("pr1_347")}
              </div>
            </AccordionSummary>
            <AccordionDetails>
              <Typography>
                <div className="toq_fil s24 mt-4 f500">
                  {getText2("pr1_353")}
                </div>
              </Typography>
            </AccordionDetails>
          </Accordion>
          <Accordion
            expanded={expanded === "panel3"}
            className="mt-2"
            onChange={handleChange("panel3")}
          >
            <AccordionSummary
              expandIcon={<ExpandMoreIcon />}
              aria-controls="panel1bh-content"
              id="panel1bh-header"
            >
              <div className="my_acc seriy s30  f600 ">
                {getText2("pr1_348")}
              </div>
            </AccordionSummary>
            <AccordionDetails>
              <Typography>
                <div className="toq_fil s24 mt-4 f500">
                  {getText2("pr1_354")}
                </div>
              </Typography>
            </AccordionDetails>
          </Accordion>
          <Accordion
            expanded={expanded === "panel4"}
            className="mt-2"
            onChange={handleChange("panel4")}
          >
            <AccordionSummary
              expandIcon={<ExpandMoreIcon />}
              aria-controls="panel1bh-content"
              id="panel1bh-header"
            >
              {" "}
              <div className="my_acc seriy s30  f600 ">
                {getText2("pr1_349")}
              </div>
            </AccordionSummary>
            <AccordionDetails>
              <Typography>
                <div className="toq_fil s24 mt-4 f500">
                  {getText2("pr1_355")} {getText2("pr1_356")}
                  <span className="f600 kok">{getText2("pr1_357")}</span>
                </div>
                <div className="toq_fil s24 mt-4 f500 ms-4">
                  {getText2("pr1_352")}
                </div>
              </Typography>
            </AccordionDetails>
          </Accordion>
          <Accordion
            expanded={expanded === "panel5"}
            className="mt-2"
            onChange={handleChange("panel5")}
          >
            <AccordionSummary
              expandIcon={<ExpandMoreIcon />}
              aria-controls="panel1bh-content"
              id="panel1bh-header"
            >
              <div className="my_acc seriy s30  f600 ">
                {getText2("pr1_350")}
              </div>
            </AccordionSummary>
            <AccordionDetails>
              <Typography>
                <div className="toq_fil s24 mt-4 f500">
                  {getText2("pr1_358")}{" "}
                  <span className="f600 color_main">{getText2("pr1_359")}</span>{" "}
                  {getText2("pr1_360")}
                  <div className="toq_fil s24 mt-4 f500">
                    {getText2("pr1_361")}
                  </div>
                  <div className="toq_fil s24 mt-4 f500">
                    {getText2("pr1_362")}
                  </div>
                </div>
              </Typography>
            </AccordionDetails>
          </Accordion>
          <div className="ta f600 kok s30 mt-5">{getText2("pr1_364")}</div>
          <div className="ta f600 kok s30 mt-1">{getText2("pr1_iltimos")}</div>
          <div className="d-flex justify-content-center mt-2">
            <FormControl>
              <FormLabel id="demo-controlled-radio-buttons-group">
                <RadioGroup className="my_radio_group">
                  <FormControlLabel
                    value="1"
                    control={
                      <Radio
                        sx={{
                          color: "silver",
                          "&.Mui-checked": {
                            color: "#FFD0D4",
                          },
                        }}
                      />
                    }
                    label="1"
                  />
                  <FormControlLabel
                    value="2"
                    control={
                      <Radio
                        sx={{
                          color: "silver",
                          "&.Mui-checked": {
                            color: "#FFD0D4",
                          },
                        }}
                      />
                    }
                    label="2"
                  />
                  <FormControlLabel
                    value="3"
                    control={
                      <Radio
                        sx={{
                          color: "silver",
                          "&.Mui-checked": {
                            color: "#FFD0D4",
                          },
                        }}
                      />
                    }
                    label="3"
                  />
                  <FormControlLabel
                    value="4"
                    control={
                      <Radio
                        sx={{
                          color: "silver",
                          "&.Mui-checked": {
                            color: "#FFD0D4",
                          },
                        }}
                      />
                    }
                    label="4"
                  />
                  <FormControlLabel
                    value="5"
                    control={
                      <Radio
                        sx={{
                          color: "silver",
                          "&.Mui-checked": {
                            color: "#FFD0D4",
                          },
                        }}
                      />
                    }
                    label="5"
                  />
                </RadioGroup>
              </FormLabel>
            </FormControl>
          </div>
          <div
            onClick={() => nav("/dashboard")}
            className="prorabotka_what tort f600 color_main s36 mt-5 ta"
          >
            {getText2("pr1_365")}
          </div>
        </div>
      </div>
    </div>
  );
};

export default Prarobotka2;
